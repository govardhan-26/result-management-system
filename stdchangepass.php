...
if(count($_POST)>0) {
$result = mysqli_query($con,"SELECT *from studentlogintable WHERE username='" . $username . "'");
$row=mysqli_fetch_array($result);
if($_POST["currentPassword"] == $row["password"] && $_POST["newPassword"] == $_POST["confirmPassword"] ) {
mysqli_query($con,"UPDATE studentlogintable set password='" . $_POST["newPassword"] . "' WHERE username='" . $username . "'");
?>
